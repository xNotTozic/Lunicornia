...
DWORD WINAPI keyThread(LPVOID lpParam) {
	...
	bool* keyMapAddr = 0x0;
	uintptr_t sigOffset = Utils::FindSignature("48 83 EC ? ? ? C1 4C 8D 05");
	if (sigOffset != 0x0) {
	...
	while (isRunning) {
		if (GameData::isKeyDown('L') && GameData::isKeyDown(VK_CONTROL) || GameData::shouldTerminate()) {  //  L to uninject
			isRunning = false;
			break;
		}
