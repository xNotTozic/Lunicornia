...
tess_vertex_t tess_vertex = reinterpret_cast<tess_vertex_t>(Utils::FindSignature("40 57 48 81 EC ? ? ? ? ? ? 7C 24")); // Updated
tess_end_t tess_end = reinterpret_cast<tess_end_t>(Utils::FindSignature("40 55 53 56 57 41 56 48 8D AC 24 ? ? ? ? 48 81 EC ? ? ? ? 49 8B F0")); // Updated
...
void DrawUtils::drawText(vec2_t pos, std::string* textStr, MC_Color *color, float textSize, Fonts font)
{
	...
	try {
		if (moduleMgr->getModule<SmoothFont>()->isEnabled())
			renderCtx->drawText(fontPtr, posF, text, color->arr, 1, 0, &size, &caretMeasureData);
		else
			renderCtx->drawDebugText(posF, text, color->arr, 1, 0, &size, &caretMeasureData);
	}
	catch (int) {}
...
{
	vec2_t textPos;
	std::string* text = new std::string(ent->getNameTag()->getText());
...
void DrawUtils::translate(vec2_t pos)
{
	current = pos;
}
...
vec2_t DrawUtils::current;
std::stack<vec2_t> DrawUtils::matrix;